...
class SessionManager:
    ...
    def _resolve_resume(
        ...
        """Decide whether this session resumes a previous one.

        Returns ``(lineage_root, auto_resume_args)``:

        * ``lineage_root`` is ``""`` for a fresh session, or the worktree *leaf*
          (a prior session's ``session_root``) to reuse so the resumed agent runs
          in the SAME directory and finds its prior, cwd-keyed conversation.
        * ``auto_resume_args`` is the agent's "continue last session" argv,
          returned only when a resume was requested but the user supplied no
          resume flag of their own (so we inject e.g. ``claude --continue``).

        A resume is triggered either explicitly (``resume_of`` — e.g. the UI's
        "Fortsetzen" button) or implicitly when the user types one of the agent's
        ``session_resume_flags`` (e.g. ``--resume <id>``) as a start parameter.
        """
        try:
            user_tokens = shlex.split(start_args) if start_args.strip() else []
        except ValueError:
            user_tokens = []
        flags = set(spec.session_resume_flags or [])
        user_typed_resume = any(tok in flags for tok in user_tokens)
        if not resume_of and not user_typed_resume:
            return "", []
        ...
        with session_scope() as db:
            target = None
            if resume_of:
                t = db.get(Task, resume_of)
                if t is not None and t.is_session and t.project_id == project_id:
                    target = t
            if target is None and user_typed_resume:
                # "Continue where I left off": the most recent ended session of
                # this project+agent.  Its lineage root (or, for sessions created
                # before this feature, its own id — which is also where it ran)
                # is the directory to reuse.
                target = (
                    db.query(Task)
                    .filter(
                        ...
                        Task.id != task_id,
                        Task.status.notin_(["running", "queued"]),
                    )
                    .order_by(Task.created_at.desc())
                    .first()
                )
            if target is not None:
                root = target.session_root or target.id
        if not root:
            return "", []
        auto = [] if user_typed_resume else list(spec.session_resume_default or [])
        return root, auto
    ...
    async def start(
        ...
        Resuming: when this session continues a previous one (``resume_of`` or a
        resume flag in ``start_args``) it runs in the prior session's worktree
        directory instead of a fresh one, because the agents key their stored
        conversations by cwd.  See ``_resolve_resume``.
        """
        import os
        import signal
        import fcntl
        ...
        # Resuming?  A resumed session must reuse the original's worktree path so
        # the agent (which keys stored conversations by cwd) finds it again; a
        # fresh session gets its own isolated path (parallel-safe).
        resume_root, resume_extra = self._resolve_resume(
            task_id, project_id, agent_key, spec, start_args, resume_of
        )
        worktree_leaf = resume_root or task_id
        if resume_root:
            ch.publish(
                {"type": "git", "data": f"[git] Setze Session fort (Worktree {worktree_leaf})\n"}
            )
        ...
        # to the main checkout if worktree setup fails.  Resumed sessions reuse
        # the original session's worktree *directory* (``worktree_leaf``) but get
        # a fresh branch, so the cwd path — and thus the agent's stored session —
        # is identical, while the new run still merges back cleanly on its own.
        run_dir = project_dir
        run_branch = base_branch
        if project_dir and Path(project_dir).exists():
            try:
                branch = f"cd/session/{task_id[:8]}"
                worktree_dir = _worktrees_root() / worktree_leaf
        ...
        with session_scope() as db:
            ...
            if task:
                task.branch = run_branch
                task.session_root = worktree_leaf
        ...
        # When resuming with no user-supplied resume flag, the agent's "continue
        # the most recent session" argv (resume_extra) is appended so the prior
        # conversation in this (reused) directory is reopened automatically.
        cmd = []
        for tok in spec.session_command:
            tok = tok.replace("{project_dir}", run_dir)
            cmd.append(tok)
        cmd += [tok.replace("{project_dir}", run_dir) for tok in resume_extra]
        if start_args.strip():
